...
class BeritaViewModel : ViewModel() {
    ...
    private val _beritaDetail = MutableLiveData<Berita>()
    val beritaDetail: LiveData<Berita> = _beritaDetail
    ...
    fun fetchBeritaDetail(id: Int) {
        _isLoading.value = true
        viewModelScope.launch {
            try {
                val response = repository.getBeritaDetail(id)
                _beritaDetail.value = response.data
                _isLoading.value = false
            } catch (e: HttpException) {
                _errorMessage.value = "Error: ${e.code()} - ${e.message()}"
                _isLoading.value = false
